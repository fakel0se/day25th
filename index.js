...
io.on('connection', function(client){
	...
	client.on('moving done', function(coordx, coordy, mX, mY){
		...
		client.broadcast.emit('moving done', users[client.id], client.id);
	...
	client.on('create train', function(coordx, coordy) {
		//console.log('train server ', client.id);
		client.emit('create train', users, coordx, coordy, client.id)
	});
